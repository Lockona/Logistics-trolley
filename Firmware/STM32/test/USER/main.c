...
rt_thread_t battery_check_task = RT_NULL;
...
//	sr04_Init();
	...
	battery_mq = rt_mq_create("battery", sizeof(rt_uint8_t), 10, RT_IPC_FLAG_FIFO);
	...
	battery_check_task = rt_thread_create("battery_check",battery_check,RT_NULL,1024,8,20);
	...
	if(battery_check_task != RT_NULL)
	{
		rt_thread_startup(battery_check_task);
	}
